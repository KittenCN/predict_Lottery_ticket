...
def check_lottery(cash_file_name, args, path_mode=1):
    ...
    with ThreadPoolExecutor(max_workers=int(args.max_workers)) as executor:
        future_to_url = {executor.submit(sub_check_lottery, cash_numpy[item_index], cash_select, cash_price, cash_list): cash_numpy[item_index] for item_index in range(len(cash_numpy))}
